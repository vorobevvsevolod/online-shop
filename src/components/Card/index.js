...
export default function Card (props){
    ...
    const onClickAddCard = (productId) => {
        ...
        if (!addedInCart) {
            dispatch(addItemInCart({product_id: props.id, quantity: 1}))
            ...
            <div className={styles.fullProduct}>
                ...
                <div className={styles.content}>
                    ...
                    <div className={styles.content_priceContainer}>
                        ...
                        {
                            console.log((cart.find(item => item.product_id === props.id)).quantity)
                        }
                        {
                            (addedInCart)
                            ?
                                <CounterProduct quantity={(cart.find(item => item.product_id === props.id)).quantity} id={props.id}/>
                            :
        ...
        return(
            <div className={styles.wrapperCard}>
                <div className={styles.card}>
                    ...
                    <img className='cu-p' width={155} height={130} src={props.image_url} alt="skeakers" onClick={() => navigate(`/product/${props.id}`)}/>
                    <h5>{props.name}</h5>
